  ...
  return (
    <AdminLayout>
      <div className="max-w-6xl mx-auto space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-[#001a33]">Registered Customers</h1>
          <p className="text-gray-500 text-sm">View all platform users and their aggregate activity</p>
        </div>

        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-4 py-4 border-b flex justify-between items-center">
            <h2 className="font-semibold text-[#001a33]">Active Consumers ({users.length})</h2>
          </div>
                  ...
                  {users.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 font-semibold text-[#001a33]">{user.name || "N/A"}</td>
                        <td className="px-4 py-3">{user.email}</td>
                        <td className="px-4 py-3 text-center font-medium">{stats.total}</td>
                        <td className="px-4 py-3 text-center">
                          {stats.active > 0 ? (
                            <span className="px-2 py-1 bg-[#001a33]/20 text-[#001a33] rounded-full text-xs font-bold">{stats.active}</span>
                          ) : (
                            <span className="text-gray-400">-</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-right font-bold text-[#001a33]">
