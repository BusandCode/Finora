  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex flex-col max-w-2xl mx-auto md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h2 className="text-3xl font-bold text-[#0A2540]">Apply for a Loan 💰</h2>
          <p className="text-[#1F2937] mt-1">Fill out the form below to request a new loan</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow p-8 max-w-2xl mx-auto">
        <form onSubmit={handleSubmit}>
          <div className="mb-6">
            <label className="block text-sm font-medium text-[#1F2937] mb-2">Loan Amount (₦)</label>
            <input
              type="number"
              required
              value={amount}
          ...
          <button
            ...
            className={`w-full mt-4 py-4 rounded-xl shadow transition font-semibold ${
              isSubmitting ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#1DBF73] text-[#0A2540] hover:bg-[#18a864]"
            }`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-10 max-w-2xl mx-auto">
        <SummaryCard title="Loan Amount" value={`₦${amount ? Number(amount).toLocaleString() : "0"}`} accent="#1DBF73" />
