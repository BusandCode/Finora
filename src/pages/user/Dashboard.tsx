  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h2 className="text-3xl font-bold text-[#0A2540]">Welcome back, {user?.email ? user.email.split("@")[0] : "User"} 👋</h2>
          <p className="text-[#1F2937]">Track your loans and repayments easily</p>
        </div>
        <button 
          onClick={() => navigate("/user/apply-loan")}
          className="mt-4 md:mt-0 bg-[#1DBF73] text-[#0A2540] px-6 py-3 rounded-lg font-semibold hover:bg-[#18a864] transition"
        >
          Apply for Loan
        </button>
      </div>
        ...
        {currentLoan ? (
          <>
            ...
            <button 
              ...
              className="mt-6 bg-[#0A2540] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[#081f35] transition"
            >
              Make Repayment
            </button>
          </>
          ...
          <div className="flex flex-col flex-1 items-center justify-center p-8 text-center bg-gray-50 rounded-xl border-dashed border-2 border-gray-200">
            ...
            <button 
              onClick={() => navigate("/user/apply-loan")}
              className="mt-4 md:mt-0 bg-[#1DBF73] text-[#0A2540] px-5 py-2.5 rounded-lg font-semibold hover:bg-[#18a864] transition"
            >
              Apply Now
            </button>
          </div>
